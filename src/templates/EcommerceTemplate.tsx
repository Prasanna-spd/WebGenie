...
const EcommerceTemplate: React.FC<EcommerceTemplateProps> = ({ title, subtitle, about, cta, showDownload, images, brand_name, brands }) => {
  const [base64HeroImages, setBase64HeroImages] = useState<string[]>([]);

  useEffect(() => {
    const loadImages = async () => {
      const base64s = await Promise.all(heroImages.map((img) => imageUrlToBase64(img)));
      setBase64HeroImages(base64s);
    };
  ...
  // const gramImages = images?.slice(3,7);
  ...
  <div class="absolute inset-0 w-full h-full">
    <div class="carousel relative h-full">
      ${base64HeroImages
        .map(
          (img, i) => `
        <div class="absolute w-full h-full carousel-slide transition-opacity duration-1000 ease-in-out ${i === 0 ? "opacity-100" : "opacity-0"}" data-index="${i}">
      ...
      <footer class="bg-gray-900 text-white px-6 py-12">
        <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12">
          <div>
            ...
            <p class="text-gray-300 text-lg leading-relaxed">${about}</p>
            <button class="mt-6 bg-white text-emerald-600 px-6 py-2 rounded-full font-semibold hover:bg-gray-100 transition">${cta}</button>
          </div>
  ...
  return (
    <div className="bg-white text-gray-900 font-sans">
      {/* --------------------------------------------------------------------------------------------------- */}
      {/* Hero Section */}
      <div className="h-12 md:h-24 text-black flex items-center justify-between px-4 lg:px-16 xl:px-20 ">
        {/* Left-side Logo/name */}
      ...
      <div className="relative h-[80vh] mx-4 lg:mx-16 rounded-2xl overflow-hidden">
        <Swiper
        ...
        >
          ...
          {base64HeroImages?.map((image, index) => (
      ...
      <div className="h-auto w-full px-4 lg:px-16 xl:px-20 py-22">
        {/* Header */}
        ...
        <div className="w-full px-4 lg:px-16 xl:px-20 py-22 text-center">
        ...
        <div className="w-full py-10 px-4 lg:px-16 text-center">
          {/* Title */}
